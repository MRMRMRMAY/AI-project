...
def data_to_image(data):
    data_image = np.fromstring(
        str(data), dtype=np.uint8, sep=' ').reshape((SIZE_FACE, SIZE_FACE)) # convert to 48 x 48 matrix
...
data = pd.read_csv(join(SAVE_DIRECTORY, DATASET_CSV_FILENAME)) # read the csv file

labels = [] # the array used to store the class labels
images = [] # the array used to store the image data
index = 1
total = data.shape[0]
for index, row in data.iterrows(): # iterate the rows of data
